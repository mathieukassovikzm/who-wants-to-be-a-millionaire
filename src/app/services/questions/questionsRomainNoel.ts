import { QuestionModel } from "@app/models/question-model"
import { TxtFin } from "@app/models/txt-fin";

export const questions = <QuestionModel[]>[
  {
    id: 0,
    gain: 100,
    question: "Que fait la vache qui rit ?",
    answers: [
      {
        id: 0,
        answer: "Elle rit",
        visible: true
      },
      {
        id: 1,
        answer: "Elle mange de l'herbe",
        visible: true
      },
      {
        id: 2,
        answer: "Elle boit du lait",
        visible: true
      },
      {
        id: 3,
        answer: "Elle fait des prouts qui sentent pas bon",
        visible: true
      }
    ],
    correctAnswer: 0
  },
  {
    id: 1,
    gain: 200,
    question: "Combien y'a t'il de chiens dans la Pat' Patrouille ?",
    answers: [
      {
        id: 0,
        answer: "4",
        visible: true
      },
      {
        id: 1,
        answer: "6",
        visible: true
      },
      {
        id: 2,
        answer: "8",
        visible: true
      },
      {
        id: 3,
        answer: "10",
        visible: true
      }
    ],
    correctAnswer: 1
  },
  {
    id: 2,
    gain: 300,
    question: "Qui est le plus grand de ces dinausores ?",
    answers: [
      {
        id: 0,
        answer: "T-rex",
        visible: true
      },
      {
        id: 1,
        answer: "Velociraptor",
        visible: true
      },
      {
        id: 2,
        answer: "Triceratops",
        visible: true
      },
      {
        id: 3,
        answer: "Stégosaure",
        visible: true
      }
    ],
    correctAnswer: 0
  },
  {
    id: 3,
    gain: 500,
    question: "Quelle est la capital du Pérou ?",
    answers: [
      {
        id: 0,
        answer: "Quito",
        visible: true
      },
      {
        id: 1,
        answer: "Bogota",
        visible: true
      },
      {
        id: 2,
        answer: "Santiago",
        visible: true
      },
      {
        id: 3,
        answer: "Lima",
        visible: true
      }
    ],
    correctAnswer: 3
  },
  {
    id: 4,
    gain: 1000,
    question: "Quel est le comble pour un électricien ?",
    answers: [
      {
        id: 0,
        answer: "Ne pas aimer le jus d'orange",
        visible: true
      },
      {
        id: 1,
        answer: "Ne pas être au courant",
        visible: true
      },
      {
        id: 2,
        answer: "N'avoir jamais visité Ampére au Brésil",
        visible: true
      },
      {
        id: 3,
        answer: "D'avoir une ampoule au pied",
        visible: true
      }
    ],
    correctAnswer: 3
  },
  {
    id: 5,
    gain: 2000,
    question: "Quel est le nom du mathématicien allemand célèbre pour son théorème sur les triangles ?",
    answers: [
      {
        id: 0,
        answer: "Euclide",
        visible: true
      },
      {
        id: 1,
        answer: "René Descartes",
        visible: true
      },
      {
        id: 2,
        answer: "Carl Friedrich Gauss",
        visible: true
      },
      {
        id: 3,
        answer: "Pythagore",
        visible: true
      }
    ],
    correctAnswer: 3
  },
  {
    id: 6,
    gain: 8000,
    question: "Qui a écrit le livre 'Le Petit Prince' ?",
    answers: [
      {
        id: 0,
        answer: "Jules Verne",
        visible: true
      },
      {
        id: 1,
        answer: "Antoine de Saint-Exupéry",
        visible: true
      },
      {
        id: 2,
        answer: "Albert Camus",
        visible: true
      },
      {
        id: 3,
        answer: "Marcel Proust",
        visible: true
      }
    ],
    correctAnswer: 1
  },
  {
    id: 7,
    gain: 12000,
    question: "Quelle est la plus haute cascade du monde ?",
    answers: [
      {
        id: 0,
        answer: "Salto Ángel",
        visible: true
      },
      {
        id: 1,
        answer: "Chutes du Niagara",
        visible: true
      },
      {
        id: 2,
        answer: "Chutes du Tugela",
        visible: true
      },
      {
        id: 3,
        answer: "Salto del Laja",
        visible: true
      }
    ],
    correctAnswer: 0
  },
  {
    id: 8,
    gain: 12000,
    question: "Qui a fondé la célèbre entreprise SpaceX ?",
    answers: [
      {
        id: 0,
        answer: "Jeff Bezos",
        visible: true
      },
      {
        id: 1,
        answer: "Bill Gates",
        visible: true
      },
      {
        id: 2,
        answer: "Elon Musk",
        visible: true
      },
      {
        id: 3,
        answer: "Mark Zuckerberg",
        visible: true
      }
    ],
    correctAnswer: 2
  },
  {
    id: 9,
    gain: 24000,
    question: "Quelle est la planète la plus éloignée du soleil dans notre système solaire ?",
    answers: [
      {
        id: 0,
        answer: "Jupiter",
        visible: true
      },
      {
        id: 1,
        answer: "Uranus",
        visible: true
      },
      {
        id: 2,
        answer: "Pluton",
        visible: true
      },
      {
        id: 3,
        answer: "Neptune",
        visible: true
      }
    ],
    correctAnswer: 3
  },
  {
    id: 10,
    gain: 36000,
    question: "Quel est le philosophe grec souvent considéré comme le père de la philosophie occidentale ?",
    answers: [
      {
        id: 0,
        answer: "Socrate",
        visible: true
      },
      {
        id: 1,
        answer: "Aristote",
        visible: true
      },
      {
        id: 2,
        answer: "Platon",
        visible: true
      },
      {
        id: 3,
        answer: "Héraclite",
        visible: true
      }
    ],
    correctAnswer: 0
  },
  {
    id: 11,
    gain: 72000,
    question: "Quelle est la distance en kilomètres du tour de la Terre à l'équateur ?",
    answers: [
      {
        id: 0,
        answer: "Environ 20,000 km",
        visible: true
      },
      {
        id: 1,
        answer: "Environ 30,000 km",
        visible: true
      },
      {
        id: 2,
        answer: "Environ 40,000 km",
        visible: true
      },
      {
        id: 3,
        answer: "Environ 50,000 km",
        visible: true
      }
    ],
    correctAnswer: 2
  },
  {
    id: 12,
    gain: 150000,
    question: "Quel est le pays avec la plus grande population en Afrique ?",
    answers: [
      {
        id: 0,
        answer: "Nigéria",
        visible: true
      },
      {
        id: 1,
        answer: "Ethiopie",
        visible: true
      },
      {
        id: 2,
        answer: "Tanzanie",
        visible: true
      },
      {
        id: 3,
        answer: "Afrique du Sud",
        visible: true
      }
    ],
    correctAnswer: 0
  },
  {
    id: 13,
    gain: 300000,
    question: "Qui a peint la fresque 'La Cène' ?",
    answers: [
      {
        id: 0,
        answer: "Michel-Ange",
        visible: true
      },
      {
        id: 1,
        answer: "Raphaël",
        visible: true
      },
      {
        id: 2,
        answer: "Leonardo da Vinci",
        visible: true
      },
      {
        id: 3,
        answer: "Sandro Botticelli",
        visible: true
      }
    ],
    correctAnswer: 2
  },
  {
    id: 14,
    gain: 1000000,
    question: "Si Mimi et Romain ont un enfant comment vont-ils l'appeler ?",
    answers: [
      {
        id: 0,
        answer: "Romanito/Romanita",
        visible: true
      },
      {
        id: 1,
        answer: "Mimito/Mimita",
        visible: true
      },
      {
        id: 2,
        answer: "Juanito/Juanita",
        visible: true
      },
      {
        id: 3,
        answer: "Pépito/Pépita",
        visible: true
      }
    ],
    correctAnswer: 3
  }
];
export const title = "Joyeux Noël";
export const birthday = "";
export const txtFin = <TxtFin>{
  resultEx: "Score parfait... Rien a dire... Ca mérite bien de boir un petit coup !",
  resultTb: "Score pas loin d'être parfait, un petit coup pour fêter ça !",
  resultBn: "Résultat tout à fait honorable mais pas de quoi se la pèter non plus... On va réviser autour d'un petit coup !",
  resultPm: "C'est pas top tout ca... On est meilleur à l'apéro... on y retoure ?",
  resultNu: "Score null à chier ! On va aller boire pour oublier !"
}